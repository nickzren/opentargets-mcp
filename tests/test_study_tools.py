...
from .conftest import TEST_DISEASE_ID_ASTHMA, TEST_STUDY_ID
...
class TestStudyTools:
    ...
    async def test_get_studies_by_disease(self, client: OpenTargetsClient):
        result = await self.study_api.get_studies_by_disease(
            client, [TEST_DISEASE_ID_ASTHMA], page_size=1
        )
        assert result is not None
    ...
    async def test_get_study_credible_sets(self, client: OpenTargetsClient):
        result = await self.study_api.get_study_credible_sets(
            client, TEST_STUDY_ID, page_size=1
        )
        assert result is not None
        assert "study" in result
        if result.get("study"):
    ...
    async def test_get_credible_set_by_id(self, client: OpenTargetsClient):
        ...
        rows = seed.get("study", {}).get("credibleSets", {}).get("rows", [])
        if not rows:
            pytest.skip("No credible sets returned for the fixture study ID.")

        study_locus_id = rows[0]["studyLocusId"]
        result = await self.study_api.get_credible_set_by_id(client, study_locus_id)
        assert result is not None
    ...
    async def test_get_credible_set_colocalisation(self, client: OpenTargetsClient):
        ...
        result = await self.study_api.get_credible_sets(
            client, study_types=["gwas"], page_size=1
        )
        assert result is not None
        assert "credibleSets" in result
        if result.get("credibleSets", {}).get("rows"):
            cs_id = result["credibleSets"]["rows"][0]["studyLocusId"]
            coloc_result = await self.study_api.get_credible_set_colocalisation(
                client, cs_id, page_size=5
            )
    ...
    async def test_get_credible_sets(self, client: OpenTargetsClient):
        result = await self.study_api.get_credible_sets(
            client, study_types=["gwas"], page_size=2
        )
        assert result is not None
        assert "credibleSets" in result
